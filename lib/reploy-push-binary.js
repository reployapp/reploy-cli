...
function createBuildZipFile() {
  ...
  if (!latestBuildPath()) {
    ...
    var buildArguments = 'xctool CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY= PROVISIONING_PROFILE=\n                          -destination platform=iOS Simulator,name=iPhone 6,OS=9.2\n                          -sdk iphonesimulator\n                          -project ios/' + projectName() + '.xcodeproj\n                          -scheme ' + projectName() + ' build';
    var buildCommand = (0, _child_process.spawnSync)('xctool', buildArguments.split(' '));
