...
var zipFileIos = '/tmp/' + projectName() + '-ios.zip';

var buildDirAndroid = _path2.default.join(process.cwd(), '/android/app/build/outputs/apk/');
...
var run = (function () {
  var ref = (0, _bluebird.coroutine)(regeneratorRuntime.mark(function _callee() {
    var application, uploadId, appetizePrivateKey, appetizeData;
...
function createBuildZipFile() {
  ...
  if (_fs2.default.existsSync(zipFileIos)) {
    _fs2.default.unlinkSync(zipFileIos);
  }

  process.chdir(buildDirIos + '/' + latestBuildPath() + '/Build/Products/Debug-iphonesimulator');
  var zip = (0, _child_process.spawnSync)('zip', ['-r', zipFileIos, '.']);
}
    ...
    return regeneratorRuntime.wrap(function _callee4$(_context4) {
      while (1) {
        switch (_context4.prev = _context4.next) {
          case 0:
            bar = new _progress2.default(':percent uploaded', { total: _fs2.default.statSync(zipFileIos).size });
            _context4.prev = 1;
            _context4.next = 4;
            return superagent.post('https://upload.uploadcare.com/base/').field('UPLOADCARE_PUB_KEY', '9e1ace5cb5be7f20d38a').attach('file', zipFileIos).on('progress', function (progress) {
              if (!bar.completed) {
