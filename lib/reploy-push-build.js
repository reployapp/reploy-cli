      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _context.next = 5;
            return (0, _postBuildUpload.uploadBuild)(platform, { buildPath: _commander2.default.buildPath, applicationId: _commander2.default.applicationId, name: _commander2.default.name });
          ...
          case 5:
            ...
            _context.prev = 7;
            _context.next = 10;
            return _api.request.post('https://api.github.com/repos/' + _commander2.default.repositoryName + '/statuses/' + _commander2.default.commitHash + '?access_token=' + _commander2.default.githubToken).send({
              "state": "success",
          ...
          case 10:
            ...
            _cli2.default.ok('Updated Github status for https://github.com/' + _commander2.default.repositoryName + '/commit/' + _commander2.default.commitHash);
...
_commander2.default.option('-p, --platform [platform]', 'Platform: "ios" or "android". REQUIRED.').option('-a, --applicationId [applicationId]', 'Your application ID on Reploy. Use \'reploy list-apps\' to get it.').option('-t, --token [token]', 'Your Reploy authentication token.').option('-b, --buildPath [buildPath]', 'Optional build file path for custom builds.').option('-s, --skip', 'Skip the build step: eiter re-upload the previous build, or upload the build file specified by -b.').option('-n, --name [name]', 'Name this build - i.e.: PR-201, v1.0, a3dffc.').option('-g, --github-token [githubToken]', 'Github token for adding a link to the Reploy build. Use only with -c.').option('-c, --commit-hash [commitHash]', 'Git commit hash whose status should be updated with the build Reploy URL.').option('-r, --repository-name [repositoryName]', 'Github repository name, i.e.: rnplay/rnplay-native.').parse(process.argv);
