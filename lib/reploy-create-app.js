...
if (_environment.appConf.app && _environment.appConf.app.id) {
  ...
  _superagent2['default'].post('http://reploy.io/api/v1/apps').set("X-ApiId", _environment.globalConf.apiId).set("X-ApiSecret", _environment.globalConf.apiSecret).send({ name: _environment.appName }).end(function (err, response) {
