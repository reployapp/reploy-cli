...
_superagent2['default'].post(url).set("X-ApiId", _environment.globalConf.apiId).set("X-ApiSecret", _environment.globalConf.apiSecret).attach('jsbundle', _path2['default'].join(process.cwd(), 'iOS/main.jsbundle')).end(function (err, res) {
  if (res.ok) {
    console.log('Done!');
